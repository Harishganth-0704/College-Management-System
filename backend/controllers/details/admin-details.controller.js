...
const registerAdminController = async (req, res, next) => {
  try {
    ...
    if (req.userId) {
      await logAction(req.userId, "AdminDetail", "CREATE_ADMIN", user._id, `Registered admin ${email}`);
    }
...
const updateDetailsController = async (req, res, next) => {
  try {
    ...
    await logAction(req.userId, "AdminDetail", "UPDATE_ADMIN", updatedUser._id, `Updated admin details for ${updatedUser.email}`);
...
const deleteDetailsController = async (req, res, next) => {
  try {
    ...
    await logAction(req.userId, "AdminDetail", "DELETE_ADMIN", req.params.id, `Deleted admin ${user.email}`);
