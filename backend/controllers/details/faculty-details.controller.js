...
const registerFacultyController = async (req, res) => {
  try {
    ...
    if (req.userId) {
      await logAction(req.userId, "AdminDetail", "CREATE_FACULTY", user._id, `Registered faculty ${email}`);
    }
...
const updateFacultyController = async (req, res) => {
  try {
    ...
    await logAction(req.userId, "AdminDetail", "UPDATE_FACULTY", updatedUser._id, `Updated faculty details for ${updatedUser.email}`);
...
const deleteFacultyController = async (req, res) => {
  try {
    ...
    await logAction(req.userId, "AdminDetail", "DELETE_FACULTY", req.params.id, `Deleted faculty ${user.email}`);
...
const getAdviseesController = async (req, res) => {
  try {
    const facultyId = req.userId;
    const advisees = await StudentDetail.find({ advisorId: facultyId }).select("-password -__v");
    return ApiResponse.success(advisees, "Advisees found").send(res);
  } catch (error) {
