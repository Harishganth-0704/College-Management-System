...
const registerStudentController = async (req, res) => {
  try {
    ...
    if (req.userId) {
      await logAction(req.userId, "AdminDetail", "CREATE_STUDENT", user._id, `Registered student ${email}`);
    }

    if (req.body.advisorId) {
      await FacultyDetail.findByIdAndUpdate(req.body.advisorId, {
        $addToSet: { advisees: user._id }
      });
    }
...
const updateDetailsController = async (req, res) => {
  try {
    ...
    if (updateData.advisorId) {
      await FacultyDetail.findByIdAndUpdate(updateData.advisorId, {
        $addToSet: { advisees: updatedUser._id }
      });
    }

    await logAction(req.userId, "AdminDetail", "UPDATE_STUDENT", updatedUser._id, `Updated student details for ${updatedUser.email}`);
...
const deleteDetailsController = async (req, res) => {
  try {
    ...
    const deletedUser = await studentDetails.findByIdAndDelete(req.params.id);

    await logAction(req.userId, "AdminDetail", "DELETE_STUDENT", req.params.id, `Deleted student ${req.params.id}`);
